...
int ChineseDate::YearDay() const
{
	...
	int month = m_month;
	if( (ChineseCalendarDB::GetLeapMonth(m_year) > 0) && (m_month > ChineseCalendarDB::GetLeapMonth(m_year) ) || ( (m_month == ChineseCalendarDB::GetLeapMonth(m_year)) && m_bLeap ) )
		month++;
...
bool ChineseDate::FromYearDay(int days)
{
    ...
    m_day = days;
	if((ChineseCalendarDB::GetLeapMonth(m_year) > 0) && (m_month > ChineseCalendarDB::GetLeapMonth(m_year)) )
	{
		m_bLeap = true;
		m_month --;
	}
